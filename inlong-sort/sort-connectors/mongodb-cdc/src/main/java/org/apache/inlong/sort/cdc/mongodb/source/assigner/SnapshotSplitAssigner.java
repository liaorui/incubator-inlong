...
package org.apache.inlong.sort.cdc.mongodb.source.assigner;
...
import org.apache.inlong.sort.cdc.mongodb.source.assigner.state.SnapshotPendingSplitsState;
...
import org.apache.inlong.sort.cdc.mongodb.source.meta.split.SchemalessSnapshotSplit;
    ...
    public void onFinishedSplits(Map<String, Offset> splitFinishedOffsets) {
        ...
        if (allSplitsFinished()) {
            ...
            if (currentParallelism == 1) {
                ...
                LOG.info(
                        "Snapshot split assigner received all splits finished and the job parallelism is 1, snapshot "
                                + "split assigner is turn into finished status.");

            } else {
                LOG.info(
                        "Snapshot split assigner received all splits finished, waiting for a complete checkpoint to "
                                + "mark the assigner finished.");
            }
        }
    }
