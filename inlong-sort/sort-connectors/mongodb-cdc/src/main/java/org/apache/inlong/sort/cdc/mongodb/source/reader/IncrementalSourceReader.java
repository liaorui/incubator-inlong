...
import org.apache.inlong.sort.cdc.mongodb.source.meta.split.SnapshotSplitState;
...
public class IncrementalSourceReader<T, C extends SourceConfig>
        extends
            SingleThreadMultiplexSourceReaderBase<SourceRecords, T, SourceSplitBase, SourceSplitState> {
    ...
    private final InlongSourceReaderMetrics sourceReaderMetrics;
    ...
    public void addSplits(List<SourceSplitBase> splits) {
        ...
        for (SourceSplitBase split : splits) {
            if (split instanceof InlongMetricSplit) {
    ...
    private void fillMetaDataForStreamSplit(StreamSplitMetaEvent metadataEvent) {
        ...
        if (streamSplit != null) {
            ...
            if (receivedMetaGroupId == expectedMetaGroupId) {
                ...
                LOG.warn(
                        "Received out of oder metadata event for split {}, the received meta group id is {}, "
                                + "but expected is {}, ignore it",
