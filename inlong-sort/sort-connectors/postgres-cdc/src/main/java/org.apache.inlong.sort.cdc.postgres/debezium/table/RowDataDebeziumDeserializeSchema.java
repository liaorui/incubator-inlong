...
package org.apache.inlong.sort.cdc.postgres.debezium.table;
...
public final class RowDataDebeziumDeserializeSchema implements DebeziumDeserializationSchema<RowData> {
     ...
     */
    private final RowKindValidator validator;

    private boolean sourceMultipleEnable;

    private ZoneId serverTimeZone;

    RowDataDebeziumDeserializeSchema(RowType physicalDataType,
            MetadataConverter[] metadataConverters,
            TypeInformation<RowData> resultTypeInfo,
            RowKindValidator validator,
        ...
        return new DeserializationRuntimeConverter() {
            ...
            public Object convert(Object dbzObj, Schema schema) {
                ...
                throw new IllegalArgumentException(
                        "Unable to convert to TimestampData from unexpected value '" + dbzObj + "' of type "
     ...
     */
    private DeserializationRuntimeConverter createConverter(LogicalType type, ZoneId serverTimeZone,
            DeserializationRuntimeConverterFactory userDefinedConverterFactory) {
     ...
     */
    public DeserializationRuntimeConverter createNotNullConverter(LogicalType type, ZoneId serverTimeZone,
            DeserializationRuntimeConverterFactory userDefinedConverterFactory) {
        // user defined converter has a higher resolve order
        Optional<DeserializationRuntimeConverter> converter = userDefinedConverterFactory.createUserDefinedConverter(
                type, serverTimeZone);
        ...
        switch (type.getTypeRoot()) {
            ...
            case ROW:
                return createRowConverter((RowType) type, serverTimeZone, userDefinedConverterFactory);
            case ARRAY:
            case MAP:
            case MULTISET:
            case RAW:
    ...
    private DeserializationRuntimeConverter createRowConverter(RowType rowType, ZoneId serverTimeZone,
            DeserializationRuntimeConverterFactory userDefinedConverterFactory) {
    ...
    private DeserializationRuntimeConverter getMultipleMigrationConverter(ZoneId serverTimeZone,
            DeserializationRuntimeConverterFactory userDefinedConverterFactory) {
        return new DeserializationRuntimeConverter() {
    ...
    private Object getTimeValue(Object fieldValue, String schemaName) {
        ...
        switch (schemaName) {
            case MicroTime.SCHEMA_NAME:
                ...
                fieldValue = timeFormatter.format(LocalDateTime.ofInstant(instant, ZoneId.of("UTC")));
                break;
            ...
            case ZonedTimestamp.SCHEMA_NAME:
                ...
                fieldValue = timestampFormatter.format(
                        zonedDateTime.withZoneSameInstant(serverTimeZone).toLocalDateTime());
                break;
            case Timestamp.SCHEMA_NAME:
                Instant instantTime = Instant.ofEpochMilli((Long) fieldValue);
                fieldValue = timestampFormatter.format(LocalDateTime.ofInstant(instantTime, ZoneId.of("UTC")));
                break;
            case MicroTimestamp.SCHEMA_NAME:
    ...
    private void emit(SourceRecord inRecord, RowData physicalRow, TableChange tableChange,
            Collector<RowData> collector) {
        if (!validator.validate(physicalRow.getRowKind())) {
            return;
        }
    ...
    public static class Builder {
        ...
        private RowKindValidator validator;
        private ZoneId serverTimeZone = ZoneId.of("UTC");
        private boolean appendSource = false;
        private boolean sourceMultipleEnable = false;
        private DeserializationRuntimeConverterFactory convertFactory = DeserializationRuntimeConverterFactory.DEFAULT;
        ...
        public Builder setValueValidator(RowKindValidator validator) {
        ...
        public Builder setUserDefinedConverterFactory(DeserializationRuntimeConverterFactory convertFactory) {
            this.convertFactory = convertFactory;
            return this;
        }

        public RowDataDebeziumDeserializeSchema build() {
            return new RowDataDebeziumDeserializeSchema(physicalRowType,
                    metadataConverters,
                    resultTypeInfo,
                    validator,
                    serverTimeZone,
