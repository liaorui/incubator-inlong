...
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.FlinkKafkaInternalProducer;
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.KafkaSerializationSchemaWrapper;
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.TransactionalIdsGenerator;
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.metrics.KafkaMetricMutableWrapper;
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.metrics.KafkaRecordOutMetricsRegistry;
...
import org.apache.flink.tdsql.streaming.util.serialization.KeyedSerializationSchema;
...
public class FlinkKafkaProducer<IN>
    ...
    protected KafkaRecordOutMetricsRegistry kafkaRecordOutMetricsRegistry;

    private long serviceDelay = 0;
    ...
    public void open(Configuration configuration) throws Exception {
        ...
        if (this.kafkaRecordOutMetricsRegistry == null) {
            MetricGroup group = getRuntimeContext().getMetricGroup();
            this.kafkaRecordOutMetricsRegistry = new KafkaRecordOutMetricsRegistry(group);
            group.gauge("sinkServiceDelay", (Gauge<Long>) () -> serviceDelay);
        }
    ...
    public void invoke(
        ...
        // c947682b
        if (record == null) {
            LOG.warn("Record is null, skip sending to kafka");
            return;
        }

        if (kafkaRecordOutMetricsRegistry != null && record != null) {
            kafkaRecordOutMetricsRegistry
                    .getBytesOutMeter()
                    .markEvent(
                            (record.key() == null ? 0 : record.key().length)
                                    + (record.value() == null ? 0 : record.value().length));
            kafkaRecordOutMetricsRegistry.getRecordOutMeter().markEvent(1);
        }

        if (context.currentWatermark() < 0) { // 未设置watermark
            serviceDelay = 0;
        } else {
            serviceDelay =
                    context.currentProcessingTime()
                            - context.currentWatermark()
                            + +LOCAL_TZ.getOffset(context.currentWatermark()); // 时区修正
            if (serviceDelay < 0) {
                LOG.debug(
                        "currentProcessingTime:{}, currentWatermark:{}, serviceDelay:{}, change to 0",
                        context.currentProcessingTime(),
                        context.currentWatermark(),
                        serviceDelay);
                serviceDelay = 0;
