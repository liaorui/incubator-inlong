...
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.metrics.KafkaRecordInMetricsRegistry;
import org.apache.flink.tdsql.streaming.connectors.kafka.internals.metrics.KafkaMetricWrapper;
...
public class KafkaConsumerThread<T> extends Thread {
    ...
    public KafkaConsumerThread(
        ...
        KafkaRecordInMetricsRegistry metricsRegistry =
                new KafkaRecordInMetricsRegistry(subtaskMetricGroup);
        this.recordsMeter = metricsRegistry.getRecordInMeter();
        this.bytesMeter = metricsRegistry.getBytesInMeter();
    }

    // ------------------------------------------------------------------------

    public double getMessagesInOneMinuteRate() {
        return this.recordsMeter.getRate();
    }

    public double getBytesInOneMinuteRate() {
        return this.bytesMeter.getRate();
    }

    private long getConsumerRecordsByteSize(ConsumerRecords<byte[], byte[]> records) {
        int result = 0;
        for (ConsumerRecord<byte[], byte[]> record : records) {
            int keyByteSize = record.key() == null ? 0 : record.key().length;
            int valueByteSize = record.value() == null ? 0 : record.value().length;
            result += keyByteSize;
            result += valueByteSize;
        }
        return result;
    }

    private <K, V> long getConsumerRecordsRecordSize(ConsumerRecords<K, V> records) {
        if (records != null) {
            ...
            while (running) {
                ...
                if (records == null) {
                    try {
                        ...
                        this.bytesMeter.markEvent(getConsumerRecordsByteSize(records));
                        this.recordsMeter.markEvent(getConsumerRecordsRecordSize(records));
