...
package org.apache.flink.tdsql.streaming.connectors.kafka.internals;
...
import org.apache.flink.tdsql.streaming.connectors.kafka.KafkaDeserializationSchema;
import org.apache.flink.tdsql.streaming.connectors.kafka.table.KafkaOptions;
...
public class KafkaFetcher<T> extends AbstractFetcher<T, TopicPartition> {
    ...
    private ByteArrayOutputStream completeMsg = new ByteArrayOutputStream();
    private final ConcurrentHashMap<String, ByteArrayOutputStream> shardMsgMap = new ConcurrentHashMap<>();
    private final ConcurrentHashMap<String, Long> shardLastSeqMap = new ConcurrentHashMap<>();
    ...
    protected void partitionConsumerRecordsHandler(List<ConsumerRecord<byte[], byte[]>> partitionRecords, KafkaTopicPartitionState<T, TopicPartition> partition) throws Exception {
        ...
        String databaseName = consumerThread.getKafkaProperties().getProperty(KafkaOptions.TDSQL_DATABASE_NAME.key());
        String tableName = consumerThread.getKafkaProperties().getProperty(KafkaOptions.TDSQL_TABLE_NAME.key());
        ...
        for (ConsumerRecord<byte[], byte[]> record : partitionRecords) {
            ...
            // When the task is restarted, the sequence id will start from 1
            // long partitionSeq = getPartitionSeq(record);
            // Each message has a shard id,
            // this id can be used to distinguish that the message belongs to which shard
            String shardId = getShardId(record);
            // Long shardLastSeq = shardLastSeqMap.get(shardId);
            LOG.debug("record partition " + record.partition() + " offset " + record.offset());
            ...
            if (1 == envelope.getTotal()) {
                ...
                LOG.info("packet start for shardId " + shardId);
            ...
            if (envelope.getIndex() < envelope.getTotal() - 1) {
                LOG.info("envelop is not complete . envelope.getIndex() < envelope.getTotal() - 1");
                continue;
            }

            byte[] valueBytesArray;
            //  SubscribeDataProto.Entries entries;
            if (1 == envelope.getTotal()) {
                // entries = SubscribeDataProto.Entries.parseFrom(completeMsg.toByteArray());
                valueBytesArray = completeMsg.toByteArray();
            } else {
                // entries = SubscribeDataProto.Entries.parseFrom(shardMsgMap.get(shardId).toByteArray());
                valueBytesArray = shardMsgMap.get(shardId).toByteArray();
                // Pay attention to memory recycling for shardMsgMap
                shardMsgMap.remove(shardId);
            }

            SubscribeDataProto.Entries entries = SubscribeDataProto.Entries.parseFrom(valueBytesArray);
            for (SubscribeDataProto.Entry entry : entries.getItemsList()) {
                // onEntry(record.partition(), record.offset(), partitionSeq, entry);
                if (filterRecord(entry, databaseName, tableName)) {
                    continue;
                }
                if (entry.getHeader().getMessageType() == SubscribeDataProto.MessageType.DML) {
                    byte[] valueBytesArray1 = entry.toByteArray();
                    ConsumerRecord<byte[], byte[]> newRecord = new ConsumerRecord<>(
                            ...
                            record.headers(),
                            record.leaderEpoch());
                    deserializer.deserialize(newRecord, kafkaCollector);

                    // emit the actual records. this also updates offset state atomically and emits
                    // watermarks
                    emitRecordsWithTimestamps(kafkaCollector.getRecords(), partition, newRecord.offset(), newRecord.timestamp());
    ...
    private static long getPartitionSeq(ConsumerRecord<byte[], byte[]> msg) {
    ...
    private static void onEntry(int partitionNum, long offset, long partitionSeq, SubscribeDataProto.Entry entry) {
        String msg = "partition:" + partitionNum + ", offset:" + offset + ", partitionSeq:" + partitionSeq
                + ", " + entry.toString();
        LOG.info("msg :" + msg);
    }

    /**
     * filter record in kafka
     *
     * @param entry        message record
     * @param databaseName databaseName defined by user
     * @param tableName    tableName defined by user
     * @return can be filtered (bool)
     */
    private static boolean filterRecord(SubscribeDataProto.Entry entry, String databaseName, String tableName) {
        String entrySchemaName = entry.getHeader().getSchemaName();
        String entryTableName = entry.getHeader().getTableName();
        boolean filtered = false;
        if (StringUtils.isNotEmpty(databaseName)) {
            if (databaseName.contains(",")) {
                String[] databases = databaseName.split(",");
                List<String> databaseList = Arrays.asList(databases);
                if (!databaseList.contains(entrySchemaName)) {
                    return true;
                }
            } else if (!Pattern.compile(databaseName).matcher(entrySchemaName).find()) {
                return true;
            }
        }
        if (StringUtils.isNotEmpty(tableName)) {
            if (tableName.contains(",")) {
                ...
                if (!tableNameList.contains(entryTableName)) {
                    return true;
                }
            } else if (!Pattern.compile(tableName).matcher(entryTableName).find()) {
                return true;
            }
        }
        return filtered;
