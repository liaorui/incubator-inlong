...
import static org.apache.flink.tdsql.streaming.connectors.kafka.table.KafkaSinkSemantic.AT_LEAST_ONCE;
import static org.apache.flink.tdsql.streaming.connectors.kafka.table.KafkaSinkSemantic.EXACTLY_ONCE;
import static org.apache.flink.tdsql.streaming.connectors.kafka.table.KafkaSinkSemantic.NONE;
...
public class KafkaOptions {
    ...
    // --------------------------------------------------------------------------------------------
    // tdsql-subscribe specific options
    // --------------------------------------------------------------------------------------------

    public static final ConfigOption<String> TDSQL_DATABASE_NAME = ConfigOptions.key("tdsql.database.name")
            .stringType()
            .defaultValue("")
            .withDescription("tdsql database name");

    public static final ConfigOption<String> TDSQL_TABLE_NAME = ConfigOptions.key("tdsql.table.name")
            .stringType()
            .defaultValue("")
            .withDescription("tdsql table name");
    ...
    public static void addTDSQLConfToProperties(ReadableConfig tableOptions, Properties properties) {
        tableOptions.getOptional(TDSQL_TABLE_NAME).ifPresent(tableName -> properties.setProperty(TDSQL_TABLE_NAME.key(), tableName));
        tableOptions.getOptional(TDSQL_DATABASE_NAME).ifPresent(databaseName -> properties.setProperty(TDSQL_DATABASE_NAME.key(), databaseName));
    }
    ...
    private static <T> FlinkKafkaPartitioner<T> initializePartitioner(
        ...
        try {
            ...
            @SuppressWarnings("unchecked") final FlinkKafkaPartitioner<T> kafkaPartitioner =
