...
public class UpsertKafkaDynamicTableFactory
    ...
    public static final String IDENTIFIER = "tdsql-subscribe-upsert";
    ...
    public DynamicTableSource createDynamicTableSource(Context context) {
        ...
        DecodingFormat<DeserializationSchema<RowData>> keyDecodingFormat =
                helper.discoverDecodingFormat(DeserializationFormatFactory.class, KafkaOptions.KEY_FORMAT);
        DecodingFormat<DeserializationSchema<RowData>> valueDecodingFormat =
                helper.discoverDecodingFormat(DeserializationFormatFactory.class, KafkaOptions.VALUE_FORMAT);
    ...
    public DynamicTableSink createDynamicTableSink(Context context) {
        ...
        EncodingFormat<SerializationSchema<RowData>> keyEncodingFormat =
                helper.discoverEncodingFormat(SerializationFormatFactory.class, KafkaOptions.KEY_FORMAT);
        EncodingFormat<SerializationSchema<RowData>> valueEncodingFormat =
                helper.discoverEncodingFormat(SerializationFormatFactory.class, KafkaOptions.VALUE_FORMAT);
        ...
        int batchSize = tableOptions.get(KafkaOptions.SINK_BUFFER_FLUSH_MAX_ROWS);
        Duration batchInterval = tableOptions.get(KafkaOptions.SINK_BUFFER_FLUSH_INTERVAL);
    ...
    private static void validateTopic(ReadableConfig tableOptions) {
        List<String> topic = tableOptions.get(KafkaOptions.TOPIC);
        if (topic.size() > 1) {
    ...
    private static void validateFormat(
        ...
        if (!keyFormat.getChangelogMode().containsOnly(RowKind.INSERT)) {
            String identifier = tableOptions.get(KafkaOptions.KEY_FORMAT);
            throw new ValidationException(
                    String.format(
        ...
        if (!valueFormat.getChangelogMode().containsOnly(RowKind.INSERT)) {
            String identifier = tableOptions.get(KafkaOptions.VALUE_FORMAT);
            throw new ValidationException(
                    String.format(
    ...
    private static void validateSinkBufferFlush(ReadableConfig tableOptions) {
        int flushMaxRows = tableOptions.get(KafkaOptions.SINK_BUFFER_FLUSH_MAX_ROWS);
        long flushIntervalMs = tableOptions.get(KafkaOptions.SINK_BUFFER_FLUSH_INTERVAL).toMillis();
        ...
        throw new ValidationException(
                String.format(
                        ...
                        KafkaOptions.SINK_BUFFER_FLUSH_MAX_ROWS.key(), KafkaOptions.SINK_BUFFER_FLUSH_INTERVAL.key()));
    }
