...
public class KafkaValidator extends ConnectorDescriptorValidator {
    ...
    public static final String CONNECTOR_TYPE_VALUE_KAFKA = "tdsql-subscribe";
    ...
    public void validate(DescriptorProperties properties) {
        ...
        properties.validateEnumValues(
                UPDATE_MODE, true, Collections.singletonList(UPDATE_MODE_VALUE_APPEND));
    ...
    private void validateStartupMode(DescriptorProperties properties) {
        ...
        specificOffsetValidators.put(
                ...
                (key) -> properties.validateInt(key, false, 0, Integer.MAX_VALUE));
        specificOffsetValidators.put(
                CONNECTOR_SPECIFIC_OFFSETS_OFFSET,
                (key) -> properties.validateLong(key, false, 0, Long.MAX_VALUE));
        ...
        if (properties.containsKey(CONNECTOR_SPECIFIC_OFFSETS)) {
            ...
            startupModeValidation.put(
                    CONNECTOR_STARTUP_MODE_VALUE_SPECIFIC_OFFSETS, noValidation());
        } else {
            startupModeValidation.put(
                    CONNECTOR_STARTUP_MODE_VALUE_SPECIFIC_OFFSETS,
                    key ->
                            properties.validateFixedIndexedProperties(
                                    CONNECTOR_SPECIFIC_OFFSETS, false, specificOffsetValidators));
        }

        startupModeValidation.put(
                CONNECTOR_STARTUP_MODE_VALUE_TIMESTAMP,
                key -> {
                    boolean hasStartupTimestampMillis =
                            properties.containsKey(CONNECTOR_STARTUP_TIMESTAMP_MILLIS);
                    if (!hasStartupTimestampMillis) {
                        throw new ValidationException(
                                String.format(
                                        "`%s` is required in timestamp startup mode but missing.",
                    ...
                    try {
                        ...
                        throw new ValidationException(
                                "Timestamp startup mode requires Kafka 0.10 or above.");
                    }
                });
    ...
    private void validateKafkaProperties(DescriptorProperties properties) {
        if (properties.containsKey(CONNECTOR_PROPERTIES_BOOTSTRAP_SERVER)
            ...
            propertyValidators.put(
                    CONNECTOR_PROPERTIES_KEY, key -> properties.validateString(key, false, 1));
            propertyValidators.put(
                    CONNECTOR_PROPERTIES_VALUE, key -> properties.validateString(key, false, 0));
            properties.validateFixedIndexedProperties(
                    CONNECTOR_PROPERTIES, true, propertyValidators);
        }
    }
     ...
     */
    public static Map<Integer, Long> validateAndParseSpecificOffsetsString(
